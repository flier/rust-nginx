...
pub fn native_handler(attr: TokenStream, handler: TokenStream) -> TokenStream {
    ...
    let expanded = handler::expand(args, handler, handler::Style::Handler);
...
#[proc_macro_error]
#[proc_macro_attribute]
pub fn native_setter(attr: TokenStream, handler: TokenStream) -> TokenStream {
    let args = parse_macro_input!(attr as handler::Args);
    let handler = parse_macro_input!(handler as syn::ItemFn);

    let expanded = handler::expand(args, handler, handler::Style::Setter);
