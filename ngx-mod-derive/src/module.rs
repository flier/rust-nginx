...
impl ToTokens for Type {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        tokens.append_all(match self {
            Self::Core(_) => quote! { ::ngx_mod::rt::ffi::NGX_CORE_MODULE },
            Self::Conf(_) => quote! { ::ngx_mod::rt::ffi::NGX_CONF_MODULE },
            #[cfg(feature = "event")]
            Self::Event(_) => quote! { ::ngx_mod::rt::ffi::NGX_EVENT_MODULE },
            #[cfg(feature = "http")]
            Self::Http(_) => quote! { ::ngx_mod::rt::ffi::NGX_HTTP_MODULE },
            #[cfg(feature = "mail")]
            Self::Mail(_) => quote! { ::ngx_mod::rt::ffi::NGX_MAIL_MODULE },
            #[cfg(feature = "stream")]
            Self::Stream(_) => quote! { ::ngx_mod::rt::ffi::NGX_STREAM_MODULE },
        })
    }
}
...
pub fn expand(input: syn::DeriveInput) -> TokenStream {
    ...
    let ngx_module: ItemStatic = parse_quote! {
        ...
        pub static mut #ngx_module_name: ::ngx_mod::rt::ffi::ngx_module_t = ::ngx_mod::rt::ffi::ngx_module_t {
            ...
            type_: #module_ty as ::ngx_mod::rt::ffi::ngx_uint_t,
    ...
    let impl_module_metadata: ItemImpl = parse_quote! {
        impl #impl_generics ::ngx_mod::ModuleMetadata for #ident #ty_generics #where_clause {
            ...
            fn commands() -> ::ngx_mod::rt::core::Cmds<'static> {
                ::ngx_mod::rt::core::Cmds::from(unsafe { & #ngx_module_cmds_name [..#ngx_module_cmds_name.len() - 1] })
            }
        }
    };
    ...
    let impl_deref: ItemImpl = parse_quote! {
        impl #impl_generics ::std::ops::Deref for #ident #ty_generics #where_clause {
            type Target = ::ngx_mod::rt::core::ModuleRef;
    ...
    let impl_as_ref: ItemImpl = parse_quote! {
        impl #impl_generics ::std::convert::AsRef<::ngx_mod::rt::core::ModuleRef> for #ident #ty_generics #where_clause {
            fn as_ref(&self) -> &::ngx_mod::rt::core::ModuleRef {
                unsafe { ::ngx_mod::rt::foreign_types::ForeignTypeRef::from_ptr(&mut #ngx_module_name as *mut _) }
            }
        }
    };
